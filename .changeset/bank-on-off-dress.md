...
The `console` kit now dresses `bank` too: selection reads as the key sitting flush (`--amb-thickness: 0`) against the rest at knob-scale thickness, with a moulded circular dish sunk into every key and a small centre LED that lights only on the selected one — a mixer-desk key bank, not a lamp-strip.
